...
if __name__ == "__main__":
	...
	print(df)
	df["index"] = df.index
	df = df.melt(id_vars=["index", "p", "exponent"], value_vars=["instance_connectivity","simpsons_index","reachability"])
	print("After melting to put in a convenient FacetGrid format:")
	print(df)
	g = sns.FacetGrid(data=df, col="variable", row="exponent", hue="variable", sharex=True, sharey=True)
	g.map(sns.scatterplot, "p", "value")
